...
- Use the job's short-lived, repository-scoped `GITHUB_TOKEN` with only `contents: write` for ref pushes. This prevents the synchronization push from launching inherited `push` workflows. Never silently fall back to a deploy key, GitHub App token, or PAT: those credentials create ordinary events and can run upstream automation inside the Fork.
- Audit the Fork's enabled workflows and recent runs after synchronization. Disable inherited build, release, deployment, or publication workflows unless the user explicitly wants them active; keep `Safe Fork Sync` active.
